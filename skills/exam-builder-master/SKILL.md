...
- 정제된 지문을 바탕으로 다음 기준에 따라 문제를 생성합니다.
  - **선지 고도화 (Advanced Options)**: 선지는 단순 단답형을 지양하고, 지문의 핵심 논리를 반영한 **중문 또는 복문 형태의 긴 문장(최소 15자 이상)**으로 구성합니다. 수능 국어/영어의 '매력적인 오답' 패턴을 적용하세요.
  - **어휘(Vocabulary)**: 문맥적 의미 파악, 유의어/반의어 치환.
  - **논리(Logic)**: 빈칸 추론, 문장 삽입, 순서 배열.
- 모든 문제의 오답(Distractors)은 정답과 유사한 구조를 갖추어야 하며, 지문의 지엽적인 정보나 논리적 비약을 교묘하게 활용하여 설계합니다.
